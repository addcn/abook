package com.dodo.android.lab.utils;
...
public class CanvasAidUtils {
    ...
    public static void drawCoordinateSystem(Canvas canvas) {
       ...
       /* CanvasUtils.drawLine(XHead, XTail, canvas, mPaint);
        ...
        canvas.save();
        canvas.drawLine(XHead.x, XHead.y, XTail.x, XTail.y, mPaint);
        canvas.restore();

        canvas.save();
        canvas.drawLine(XHead.x, XHead.y, XCap1.x, XCap1.y, mPaint);
        canvas.restore();

        canvas.save();
        canvas.drawLine(XHead.x, XHead.y, XCap2.x, XCap2.y, mPaint);
        canvas.restore();


        canvas.save();
        canvas.drawLine(YHead.x, YHead.y, YTail.x, YTail.y, mPaint);
        canvas.restore();

        canvas.save();
        canvas.drawLine(YHead.x, YHead.y, YCap1.x, YCap1.y, mPaint);
        canvas.restore();

        canvas.save();
        canvas.drawLine(YHead.x, YHead.y, YCap2.x, YCap2.y, mPaint);
        canvas.restore();
